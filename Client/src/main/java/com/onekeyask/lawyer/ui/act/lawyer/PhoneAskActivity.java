...
import com.onekeyask.lawyer.entity.PhoneServiceInfo;
...
public class PhoneAskActivity extends BaseToolBarActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        localReceiver = new PhoneAskActivity.LocalReceiver();
    ...
    private void initBalance() {
        ...
        retrofitUtil.getPriceList(service.getUserId(), new ProgressSubscriber<PriceList>(getResultOnNext, PhoneAskActivity.this, true));
    ...
    private void initData() {
        ...
        SubscriberOnNextListener<LawyerBasic> listener = new SubscriberOnNextListener<LawyerBasic>() {
            ...
            public void onNext(LawyerBasic lawyerBasic) {
                ...
                Glide.with(PhoneAskActivity.this).load(lawyerBasic.getLawyer().getHeadURL())
        ...
        retrofitUtil.getLawyerBasic(lawyerId, new ProgressSubscriber<LawyerBasic>(listener, PhoneAskActivity.this, true));


        OkGo.<String>get(Apis.TelChatServiceInfo)
                .params("lawyerId", lawyerId)
                .params("userId", UserService.service(getBaseContext()).getUserId())
                .execute(new StringCallback() {
                    @Override
                    public void onSuccess(Response<String> response) {
                        PhoneServiceInfo info = (new Gson()).fromJson(response.body(), PhoneServiceInfo.class);
                        if (info.getCode() == 0) {

                            service_name.setText(info.getData().getTelChat().getServiceName());

                            money = info.getData().getTelChat().getPriceList().get(0).getPrice();
                            priceOnce.setText("￥" + info.getData().getTelChat().getPriceList().get(0).getPrice()
                                    + "元/" + info.getData().getTelChat().getPriceList().get(0).getCycle());
                            totalPrice.setText(UserService.service(getBaseContext()).getPhone());

                            priceId = info.getData().getTelChat().getPriceList().get(0).getPriceId();
...
//                                initBalance();
    ...
    private void goAliPay(final String orderInfo) {
        ...
        Runnable payRunnable = new Runnable() {
            ...
            @Override
            public void run() {
                PayTask alipay = new PayTask(PhoneAskActivity.this);
...
//                goNext(oid + "", fid + "");
    ...
    private void goNext() {
        Intent intent = new Intent(PhoneAskActivity.this, CallDetailActivity.class);
        intent.putExtra("userServiceId", "0");
    ...
    private PhoneAskActivity.LocalReceiver localReceiver;
