...
//        retrofitUtil.getFoundFragList(, index, size, new ProgressSubscriber<FoundFrag>(listener, getActivity(), false));
    ...
    private class DiscoverAdapter extends RecyclerView.Adapter<DiscoverAdapter.ViewHolder> {
        ...
        public void onBindViewHolder(ViewHolder holder, final int position) {
            ...
            if (!data.get(position).getMoney().equals("0.0"))
            holder.dis_money.setText("￥ "+data.get(position).getMoney());
            holder.dis_office.setText(data.get(position).getCreateTime());
//            holder.dis_count.setText(String.valueOf(data.get(position).getSupportCount()));
            ...
            if (data.get(position).getPicList().size() == 0){
                holder.dis_iv1.setVisibility(View.GONE);
                holder.dis_iv2.setVisibility(View.GONE);
                holder.dis_iv3.setVisibility(View.GONE);
            }else if (data.get(position).getPicList().size()==1){
                holder.dis_iv1.setVisibility(View.VISIBLE);
                Glide.with(getActivity()).load(data.get(position).getPicList().get(0)).into(holder.dis_iv1);
                holder.dis_iv2.setVisibility(View.GONE);
                holder.dis_iv3.setVisibility(View.GONE);
            }else if (data.get(position).getPicList().size()==2){
                holder.dis_iv1.setVisibility(View.VISIBLE);
                Glide.with(getActivity()).load(data.get(position).getPicList().get(0)).into(holder.dis_iv1);
                holder.dis_iv2.setVisibility(View.VISIBLE);
                Glide.with(getActivity()).load(data.get(position).getPicList().get(1)).into(holder.dis_iv2);
                holder.dis_iv3.setVisibility(View.GONE);
            }else if (data.get(position).getPicList().size()==3){
                holder.dis_iv1.setVisibility(View.VISIBLE);
                Glide.with(getActivity()).load(data.get(position).getPicList().get(0)).into(holder.dis_iv1);
                holder.dis_iv2.setVisibility(View.VISIBLE);
                Glide.with(getActivity()).load(data.get(position).getPicList().get(1)).into(holder.dis_iv2);
                holder.dis_iv3.setVisibility(View.VISIBLE);
                Glide.with(getActivity()).load(data.get(position).getPicList().get(2)).into(holder.dis_iv3);
            }
...
//                    startActivity(AskDetailActivity.class, "freeAskId", data.get(position).getFreeaskId() + "");
        ...
        class ViewHolder extends RecyclerView.ViewHolder {
            ...
            private TextView dis_type, dis_context, dis_name, dis_office, dis_count, dis_money;
            private ImageView dis_praise, dis_iv1, dis_iv2, dis_iv3;
            ...
            public ViewHolder(View itemView) {
                ...
                dis_iv1 = (ImageView) itemView.findViewById(R.id.dis_iv1);
                dis_iv2 = (ImageView) itemView.findViewById(R.id.dis_iv2);
                dis_iv3 = (ImageView) itemView.findViewById(R.id.dis_iv3);
                dis_img = (CircleImageView) itemView.findViewById(R.id.dis_img);
                dis_money = (TextView) itemView.findViewById(R.id.dis_money);
            }
        }
